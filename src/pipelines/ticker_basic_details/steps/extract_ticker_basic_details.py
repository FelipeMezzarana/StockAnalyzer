...
class TickerBasicDetailsExtractor(Step):
    ...
    def get_required_tickers(self) -> list[str]:
        ...
        _, all_tickers = self.sqlite_client.query(
            f"SELECT DISTINCT(exchange_symbol) FROM GROUPED_DAILY"
            )
        
        _, registered_tickers = self.sqlite_client.query(
            f"SELECT DISTINCT(exchange_symbol) FROM TICKER_BASIC_DETAILS"
            )
        registered_tickers_list = [r[0] for r in registered_tickers]
        required_tickers = [
            t[0] for t in all_tickers 
            if t[0] not in registered_tickers_list
            ]
        self.logger.info(
    ...
    def update_ticker_basic_details(self, required_tickers: list[str], registered_tickers: list[str]):
        ...
        header = list(self.settings.PIPELINE_TABLE.get("fields_mapping").keys())
        ...
        while required_tickers:
                ...
                # filter registred
                required_data = [t for t in data if t.get("ticker") not in registered_tickers]
                # Update requirements 
                ...
                if not next_url: # pragma: no cover
                    ...
                    break
                if api_call_count > self.max_paginagion:
