...
class SP500Checker(Step):
    ...
    def get_last_update(self) -> datetime:
        ...
        if is_successful:
            ...
            if last_update:
                ...
                self.logger.info(f"SP500_BASIC_DETAILS Table empty.")
                return datetime(1990,1,1)
        else: # pragma: no cover
            raise KeyError(f"SP500_BASIC_DETAILS Table no found.")
    ...
    def run(self):
        ...
        if (today - last_update).days > 30:
            ...
            self.logger.info(
                f"Table not updated in past 30 days. Running pipeline."
                )
        else:
            self.output["skip_pipeline"] = True
            self.logger.info(
                f"Table up to date. Updated {(today - last_update).days} days ago. Skipping pipeline"
                )
